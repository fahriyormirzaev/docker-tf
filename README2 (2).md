...
- Run the second `ubuntu` os with interactive shell open and name container as `tch` and show that this `ubuntu` container is different from the previous one.

```bash
docker run -i -t --name tch ubuntu
```
...
- Restart the `tch` container by its name and list only running containers.

```bash
docker start tch && docker ps
```

- Connect to the interactive shell of running `tch` container and `exit` afterwards.

```bash
docker attach tch
```

- Show that `tch` container has stopped by listing all containers.
...
- Show that we can get more information about `tch` container by using `docker inspect` command and explain the properties.

```bash
docker inspect tch | less
```
